...
This module provides the main CLI interface for running parsers, including
...
from data_parser_app.health import create_health_app
from data_parser_core.core import DataRegistryParserConfig
from data_parser_core.logging import (
    ConsoleMode,
    LoggingHandler,
    LoggingLevel,
    configure_logging,
)
from data_parser_core.parser import run_parser
...
def generate_run_id(config_id: str) -> str:
    ...
    Returns:
        A unique run ID in the format: parser_{config_id}_{timestamp}
    """
    timestamp = datetime.now(tz=UTC).strftime("%Y%m%d%H%M%S")
    return f"parser_{config_id}_{timestamp}"


# ruff: noqa: PLR0912
def run_command(args: list[str] | None = None) -> None:
    """Run a data parser with the specified configuration.

    This command executes a data parser using the provided configuration ID.
    The parser will process data according to its configured rules and output
...
def show_help() -> None:
    ...
    help_text = """
OpenCorporates Data parser

Usage:
    python -m data_parser_app.main <command> [options]

Commands:
    run                     Run a data parser (uses environment variables for config)
...
Examples:
    ...
    python -m data_parser_app.main run

    # Using command line arguments (all three required together)
    python -m data_parser_app.main run --data-registry-id us-fl --stage raw --config-dir ./mocks/us_fl/config
    python -m data_parser_app.main run --data-registry-id us-fl --stage raw --config-dir ./mocks/us_fl/config --storage file
    python -m data_parser_app.main health --port 8080
"""
    print(help_text)
...
def main() -> None:
    ...
    dispatch = {
        ...
        "-h": show_help,
        "help": show_help,
        "--version": lambda: print("data-parser-app, version 0.1.0"),
        "-v": lambda: print("data-parser-app, version 0.1.0"),
        "version": lambda: print("data-parser-app, version 0.1.0"),
    }
...
async def main_async(args: dict[str, Any]) -> None:
    ...
    with log_bind(run_id=run_id, data_registry_id=data_registry_id):
        ...
        # Create parser configuration with CLI arguments
        with observe_around(logger, "CREATE_parser_APP_CONFIG"):
            app_config = await create_parser_app_config(
                data_registry_id=data_registry_id,
                credentials_provider_type=args["credentials_provider"],
                storage_type=args["storage"],
        ...
        try:
            plan = None
            with observe_around(logger, "INITIALIZE_parser"):
                logger.info("parser_INITIALIZING", data_registry_id=data_registry_id)
                ...
                # Load YAML parser configuration directly via DataPipelineConfig
