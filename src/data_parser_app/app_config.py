...
from data_parser_core.kv_store import (
    create_kv_store,
)
from data_parser_core.storage import Storage, create_storage_config_instance
...
async def create_parser_app_config(
...
@environ.config(prefix="DATA_parser_APP")
class RunConfig:
    ...
    stage: str | None = environ.var(
        default=None,
        help="Pipeline stage (e.g., parser). Required if --data-registry-id is specified.",
    )
    step: str | None = environ.var(
        default=None,
        help="Pipeline step used for config selection (e.g., parser). "
