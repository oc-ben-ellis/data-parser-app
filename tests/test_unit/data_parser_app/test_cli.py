...
from data_parser_app.main import (
    generate_run_id,
    health_command,
    main,
    run_command,
...
class TestCLI:
    ...
    def test_generate_run_id(self) -> None:
        ...
        assert run_id.startswith("parser_test_config_")
        assert len(run_id) > len("parser_test_config_")
    ...
    def test_show_help(self, capsys) -> None:
        ...
        assert "OpenCorporates Data parser" in captured.out
    ...
    def test_main_with_version(self, capsys) -> None:
        ...
        captured = capsys.readouterr()
        assert "data-parser-app, version 0.1.0" in captured.out
    ...
    @patch("data_parser_app.main.create_run_config")
    @patch("data_parser_app.main.configure_logging")
    @patch("data_parser_app.main.main_async")
    def test_run_command_basic(
        self, mock_main_async, mock_configure_logging, mock_create_run_config
    ) -> None:
        ...
        with patch.dict(
            "os.environ", {"OC_DATA_PIPELINE_DATA_REGISTRY_ID": "test_registry"}
        ):
            with pytest.raises(SystemExit) as exc_info:
    ...
    @patch("data_parser_app.main.create_health_config")
    @patch("data_parser_app.main.configure_logging")
    @patch("data_parser_app.main.create_health_app")
    @patch("data_parser_app.main.make_server")
    def test_health_command_basic(
        self,
        mock_make_server,
        mock_create_health_app,
        mock_configure_logging,
        mock_create_health_config,
    ) -> None:
    ...
    def test_run_command_missing_stage_when_data_registry_id_provided(
        self, capsys
    ) -> None:
        """Test run command with data registry ID but missing stage."""
        with patch("data_parser_app.main.create_run_config") as mock_create_run_config:
            mock_config = MagicMock()
    ...
    def test_run_command_missing_step_when_data_registry_id_provided(
        self, capsys
    ) -> None:
        ...
        with patch("data_parser_app.main.create_run_config") as mock_create_run_config:
            ...
            with pytest.raises(SystemExit) as exc_info:
                run_command(
                    ["--data-registry-id", "test_registry", "--stage", "test_stage"]
                )
            assert exc_info.value.code == 1
