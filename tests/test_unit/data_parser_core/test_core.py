...
class TestParseRunContext:
    """Test ParseRunContext dataclass."""

    def test_basic_creation(self) -> None:
        """Test basic ParseRunContext creation."""
        ctx = ParseRunContext(run_id="test_run")
        assert ctx.shared == {}

    def test_with_shared_data(self) -> None:
        """Test ParseRunContext with shared data."""
        ctx = ParseRunContext(run_id="test_run", shared={"key": "value", "count": 42})
        assert ctx.shared == {"key": "value", "count": 42}

    def test_with_run_id(self) -> None:
        """Test ParseRunContext with run_id."""
        run_id = "parser_test_20250127143022"
        ctx = ParseRunContext(run_id=run_id)
        assert ctx.run_id == run_id
        assert ctx.shared == {}

    def test_with_run_id_and_shared_data(self) -> None:
        """Test ParseRunContext with both run_id and shared data."""
        run_id = "parser_test_20250127143022"
        shared_data = {"key": "value", "count": 42}
        ctx = ParseRunContext(run_id=run_id, shared=shared_data)
        assert ctx.run_id == run_id
        assert ctx.shared == shared_data


class TestParsePlan:
    """Test ParsePlan dataclass."""
